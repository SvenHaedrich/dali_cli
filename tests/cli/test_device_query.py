...
def test_query_device_command(command, opcode):
    ...
    for short in range(DaliMax.ADR):
        result = runner.invoke(
            cli, ["--mock", "device", "query", command, "--adr", str(short)]
        )
    ...
    for group in range(DaliMax.DEVICE_GROUP):
        result = runner.invoke(
            cli, ["--mock", "device", "query", command, "--adr", f"G{group}"]
        )
...
def test_query_device_instance_command(command, opcode):
    ...
    result = runner.invoke(
        cli, ["--mock", "device", "query", command, "--instance", "BC"]
    )
    ...
    result = runner.invoke(
        cli, ["--mock", "device", "query", command, "--instance", "DEVICE"]
    )
    ...
    for group in range(DaliMax.INSTANCE_GROUP):
        result = runner.invoke(
            cli, ["--mock", "device", "query", command, "--instance", f"G{group}"]
        )
    ...
    for typ in range(DaliMax.INSTANCE_TYPES):
        result = runner.invoke(
            cli, ["--mock", "device", "query", command, "--instance", f"T{typ}"]
        )
